...
const GeneralSignals = () => {
  ...
  // Mock data for signals with creators
  useEffect(() => {
    const mockSignals = [
      {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(signal => 
        ...
        signal.creator.name.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    ...
    if (filterType !== 'all') {
      if (filterType === 'stocks') {
        ...
        filtered = filtered.filter(signal => signal.creator.verified);
      }
    }
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg p-6 mb-6 border border-gray-700">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search signals or creators..."
                value={searchTerm}
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-600">
                  <div className="flex items-center gap-3">
                    <img
                      src={signal.creator.image}
                      alt={signal.creator.name}
                      className="w-12 h-12 rounded-full object-cover ring-2 ring-gray-600"
                    />
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="text-white font-semibold">{signal.creator.name}</h4>
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-400">
                        <div className="flex items-center gap-1">
                          ...
                          {signal.creator.rating.toFixed(1)}
                        </div>
                        <div className="flex items-center gap-1">
                          <Users size={14} />
                          {formatFollowers(signal.creator.followers)} followers
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-8 gap-4 items-center">
                  ...
                  <div className="text-center">
                    {signal.status === 'Open' ? (
                      <button className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold text-sm transition-colors duration-200 flex items-center gap-2 mx-auto">
                        <DollarSign size={14} />
                        Trade Now
                      </button>
                    ) : (
